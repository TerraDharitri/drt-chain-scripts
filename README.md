# Dharitri Node Deploy Scripts

## INTRODUCTION

The current scripts version aims to bring the validator experience to a higher standard.
This variant of the scripts can be used on any of the Dharitri Networks (mainnet,testnet or devnet) by setting the new `ENVIRONMENT` variable.
The `ENVIRONMENT` variable will direct the scripts to use the correct drt-chain-configs repositories (drt-chain-config-mainnet, drt-chain-config-testnet, or drt-chain-config-devnet).
Running an observing squad (4 nodes + proxy) is also supported on all Dharitri Networks.
...
- Running the script requires a user (not root) with sudo priviledges (without password). Find more information here: <https://docs.dharitri.org/validators/nodes-scripts/config-scripts/#ensure-user-privileges>
...
mv validatorKey.pem ~/numbat-nodes/node-0/config
```
...
## RUNNING THE SCRIPT
    ...
    [INSTALL OBSERVING SQUAD]    
        #installs four observing node(s) and an numbat proxy instance on the local machine
        ./script.sh observing_squad

        Running the script with the 'observing_squad' parameter will deploy four observers (one for each shard) plus an instance of the Dharitri Proxy
            - please make sure your machine is able to comfortably run in such a configuration 
    ...
    [UPGRADE]
        ...
        ./script.sh upgrade_proxy - whenever you need to update the Dharitri Proxy instance (in the observing squad configuration)
...
This version of scripts will start your nodes as separate systemd services and an additional termui binary will be build for you on each machine and placed in your $CUSTOM_HOME/numbat-utils folder.
...
During the install process your nodes will have rest api sockets assigned to them following this pattern:
    ...
    numbat-node-0 will use localhost:8080
    numbat-node-1 will use localhost:8081
    numbat-node-2 will use localhost:8082
    ...
    numbat-node-x will use localhost:(8080+x)

You can check the status of each of your nodes in turn by going to your $CUSTOM_HOME/numbat-utils/ folder and using this command (making sure you select the proper socket for the desired node):

    ./numbat-utils/termui -address localhost:8080
     or
    ./numbat-utils/termui -address localhost:8081
    ...

## LOGVIEWER INFO

This version of scripts will start your nodes as separate systemd services and an additional logviewer binary will be build for you on each machine and placed in your $CUSTOM_HOME/numbat-utils folder.
This tool provides a way of capturing (and even storing) logger lines generated by an numbat-node instance. The binary will try to connect to the node over the rest API interface provided.
During the install process your nodes will have rest api sockets assigned to them following this pattern:
...
You can check the status of each of your nodes in turn by going to your $CUSTOM_HOME/numbat-utils/ folder and using this command (making sure you select the proper socket for the desired node):
    ...
    ./numbat-utils/logviewer -address localhost:8080
    or
    ./numbat-utils/logviewer -address localhost:8081
    ...
...
The flag for storing into a file the received logger lines is  `-log-save`
  ...
  Example:
          `./numbat-utils/logviewer -address localhost:8080 -level *:DEBUG,api:INFO -log-save` will start the binary that will try to connect to the locally-opened 8080 port, will set the log level
      to DEBUG for all packages except api package and will store all captured log lines in a file.

## SEEDNODE INFO

If there's ever a need to start a separate seed/boot node - there's a seednode binary available in the $CUSTOM_HOME/numbat-utils/seednode/ folder.
...
As of version 1.3.4 of the scripts there is a new command for extracting and tarballing your node logs. The tar file will be stored in the $CUSTOM_HOME/numbat-logs folder and will contain a timestamp in their name.
...
## FINAL THOUGHTS
   ...
   KEEP CALM AND VALIDATE ON THE Dharitri NETWORK!
